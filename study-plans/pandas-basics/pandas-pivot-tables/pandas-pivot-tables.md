...
<span style="font-size: 14px;">`margins=True` adds a row and column of totals. Margins apply the aggregation to the underlying records, not to already aggregated cells. This is equivalent to Excel's "Grand Total" row and column.</span>
...
pd.pivot_table(df, values='revenue', index='dept', columns='quarter', aggfunc='sum', fill_value=0)
...
* <span style="font-size: 14px;">**Margin aggregation**: Margins apply the same function as the cells. For a sum table, margins are sums. For a mean table, each margin is the mean of its underlying records; the grand total is the mean of all included records.</span>