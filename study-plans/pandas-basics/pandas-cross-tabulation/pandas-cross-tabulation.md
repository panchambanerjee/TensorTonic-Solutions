...
<span style="font-size: 14px;">`crosstab()` is syntactic sugar: `pd.crosstab(s1, s2)` is equivalent to pd.pivot_table with s1 as index, s2 as columns, size as aggfunc, and fill_value set to zero. Use whichever reads more clearly.</span>
