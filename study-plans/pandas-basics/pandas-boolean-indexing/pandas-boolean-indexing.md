...
<span style="font-size: 14px;">Equality compares string values; ordering comparisons use lexicographic order. For pattern matching, use `.str.contains()` or `.str.startswith()`.</span>
...
<span style="font-size: 14px;">For ordinary floating-point NaN, ordered comparisons and equality are false, but inequality is true:</span>
...
<span style="font-size: 14px;">The greater-than filter excludes NaN rows. A not-equal filter can retain them, so use isna or notna when missing values need explicit handling:</span>
...
* <span style="font-size: 14px;">**NaN comparisons**: Greater-than filters exclude NaN, but not-equal filters can retain it. Handle missing values explicitly when needed.</span>
