...
<span style="font-size: 14px;">With `iloc`, the number is always a position. With `loc`, the number is always a label. After filtering, labels may be [0, 2, 5]. Label 2 then identifies the second row, while position 2 identifies the third row.</span>
