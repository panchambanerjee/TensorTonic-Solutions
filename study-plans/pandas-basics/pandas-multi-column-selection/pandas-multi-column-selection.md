...
Sets do not preserve requested column order; avoid this approach when order matters.
...
<span style="font-size: 14px;">Multi-column selection creates a new DataFrame. Storage sharing depends on the pandas version and copy-on-write settings:</span>

* <span style="font-size: 14px;">**Copying may occur**: Selecting a subset can allocate new data rather than share the original storage.</span>
* <span style="font-size: 14px;">**Copy-on-write is conditional**: When enabled, writing to shared data triggers a copy.</span>
* <span style="font-size: 14px;">**Cost depends on the selection**: Column dtypes, selected sizes and storage layout affect time and memory use.</span>
